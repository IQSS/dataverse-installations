...
def get_selected_dataverse_ids(dv_alias_list, include_child_dvs=True):
    ...
    full_tree = get_dataverse_tree_dict()

    # Check through tree, looking
    additional_ids = get_subtree_child_ids(full_tree, first_cut_id_list)

    return additional_ids


def get_subtree_child_ids(tree_info, selected_ids, id_list=None, gather_all=False):
    """
    ...
    if tree_info['id'] in selected_ids or gather_all:
        ...
        for child_tree in tree_info.get('children', []):
            get_subtree_child_ids(child_tree, selected_ids, id_list, gather_all=True)
    else:
        # Nope, just keep checking
        for child_tree in tree_info.get('children', []):
            get_subtree_child_ids(child_tree, selected_ids, id_list)

    return id_list



def get_dataverse_tree_dict(skip_flat_dataverses=True):
